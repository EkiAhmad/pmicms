                ...
                <div class="card shadow">
                    <div class="card-header bg-transparent">
                        <div class="row align-items-center">
                            <div class="col">
                                <h6 class="text-uppercase text-muted ls-1 mb-1">Update Data Data Galeri</h6>
                    ...
                    <div class="card-body">
                        <form action="{{ route($route.'update_action') }}" class="form-global-handle" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="galeri_id" value="{{ Hashids::encode($data->galeri_id) }}">

                            <div class="form-group">
                                <label for="Kegiatan">Kegiatan</label>
                                <select name="kegiatan_id" id="Kegiatan" class="form-control">
                                    <option value="{{ $data->kegiatan->kegiatan_id }}">{{ $data->kegiatan->kegiatan_title }}</option>
                                    @foreach ($kegiatan->where('kegiatan_id','!=', $data->kegiatan_id) as $key => $value)
                            ...
                            {{-- <div class="form-group"> --}}
                                ...
                                <input type="hidden" name="galeri_location" id="Image" value="{{ $data->galeri_location }}">
                            {{-- </div> --}}
