                    ...
                    <div class="card-body">
                        <form action="{{ route($route.'update_action') }}" class="form-global-handle" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" value="{{ $data->name }}" class="form-control" id="RoleName" placeholder="Example : Admin">
                            </div>
                            <div class="form-group">
                                    ...
                                    @foreach ($permission as $key => $value)
                                        <div class="col-lg-2">
                                            @foreach ($value as $key_value => $value_data)
                                                ...
                                                <input class="custom-control-input" name="permission_id[]" value="{{ $value_data->id }}" id="customCheck{{$key.$key_value}}" {{ in_array($value_data->id, $data->permission) ? 'checked' : null }} type="checkbox">
