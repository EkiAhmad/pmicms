                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form action="{{ route($route.'update_action') }}" class="form-global-handle" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="user_id" value="{{ Hashids::encode($data->user_id) }}">
                            <div class="form-group">
                                <label for="RoleName">Username</label>
                                <input type="text" name="user_username" required class="form-control" value="{{ $data->user_username }}" id="RoleName" placeholder="Example : Admin">
                            </div>
                            <div class="form-group">
                                ...
                                <div class="row">
                                    @foreach ($udd as $key => $value)
                                        <div class="col-lg-12">
                                            ...
                                            <input class="custom-control-input" name="udd_id" value="{{ $value->id }}" id="udd_id{{ $key }}" {{ $value->id == $data->udd ? 'checked' : null }} type="radio" required>
                                    ...
                                    @foreach ($role as $key => $value)
                                        <div class="col-lg-2">
                                            @foreach ($value as $key_value => $value_role)
                                                ...
                                                <input class="custom-control-input" name="role_id[]" value="{{ $value_role->id }}" id="customCheck{{$key.$key_value}}" {{ in_array($value_role->id, $data->role) ? 'checked' : null }} type="checkbox">
