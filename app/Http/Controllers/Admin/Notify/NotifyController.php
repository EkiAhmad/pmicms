...
class NotifyController extends Controller
{
    ...
    * Silahkan ganti 
    * Notify:: => dengan model anda
    * $id_notify => ganti dengan id di model anda
    */
     ...
     */
    private $title = 'App | Notify'; /**jangan lupa diganti*/
    private $route = 'admin.notify.'; //path awal foldernya ajah (misal folder di admin/dashboard) => 'admin.dashboard' | jangan lupa diganti
    ...
    public function getData()
    {
        ...
        return Datatables::of($query)
            ->addColumn('aksi', function (Notify $notify) {
                return "
                        <a href='" . route($this->route . 'update', ['notify_id' => Hashids::encode($notify->data_notify_id)]) . "' class='btn btn-sm btn-primary btn-edit'>Edit</a>
                        <a href='javascript:;' data-route='" . route($this->route . 'delete_action', ['notify_id' => Hashids::encode($notify->data_notify_id)]) . "' class='btn btn-danger btn-sm btn-delete'>Delete</a>
                       ";
            })
            ->rawColumns(['aksi'])
            ->toJson();
    ...
    */
    ...
    public function create_action(Request $request)
    {
        ...
        $rules = [
            'data_notify_judul'     => 'required',
            'data_notify_content'   => 'required'
        ];
    ...
    public function update($id_notify)
    {
        ...
        $id_notify = Hashids::decode($id_notify);
        if (!empty($id_notify)) {

            $cek_data = Notify::where('data_notify_id', $id_notify[0])->first();
            // dd($cek_data);
    ...
    public function update_action(Request $request)
    {
        ...
        $id_notify = Hashids::decode($request['data_notify_id']);
     ...
     */
    public function delete_action($id_notify)
    {
        $id_notify = Hashids::decode($id_notify);
