...
class KegiatanController extends Controller
{
    ...
    * Silahkan ganti 
    * DataKegiatan:: => dengan model anda
    * $kegiatan_id => ganti dengan id di model anda
    */
     ...
     */
    private $title = 'App | Data - Kegiatan'; /**jangan lupa diganti*/
    private $route = 'admin.data.kegiatan.'; //path awal foldernya ajah (misal folder di admin/dashboard) => 'admin.dashboard' | jangan lupa diganti
    ...
    public function getData()
    {
        ...
        $query = DataKegiatan::where('kegiatan_status', '=', 'Y')->get();
        foreach ($query as $key => $value) {
            $value->no = $key + 1;
        }
        // dd($query);
        return Datatables::of($query)
            ->addColumn('aksi', function (DataKegiatan $dataKegiatan) {
                return "
                        <a href='" . route($this->route . 'update', ['kegiatan_id' => Hashids::encode($dataKegiatan->kegiatan_id)]) . "' class='btn btn-sm btn-primary btn-edit'>Edit</a>
                        <a href='javascript:;' data-route='" . route($this->route . 'delete_action', ['kegiatan_id' => Hashids::encode($dataKegiatan->kegiatan_id)]) . "' class='btn btn-danger btn-sm btn-delete'>Delete</a>
                       ";
            })
            ->addColumn('type', function (DataKegiatan $dataKegiatan) {
                if ($dataKegiatan->kegiatan_type == 'P') {
                    return "Tertutup";
                } else {
                    return "Umum";
                }
    ...
    */
    ...
    public function create_action(Request $request)
    {
        ...
        $rules = [
            'kegiatan_title'    =>  'required',
            'tgl_mulai'         =>  'required',
            'tgl_selesai'       =>  'required',
            'kegiatan_content'  =>  'required',
            'kegiatan_type'     =>  'required',
            'lat'               =>  'required',
            'lng'               =>  'required',
            'lokasi'            =>  'required',
            'kegiatan_image'    =>  'required',
            'kegiatan_estimasi' =>  'required',
        ...
        if ($validator->passes()) {
            ...
            $query = DataKegiatan::create($insert);
    ...
    public function update($kegiatan_id)
    {
        ...
        $kegiatan_id = Hashids::decode($kegiatan_id);
        if (!empty($kegiatan_id)) {

            $cek_data = DataKegiatan::where('kegiatan_id', $kegiatan_id[0])->first();
    ...
    public function update_action(Request $request)
    {
        ...
        $kegiatan_id = Hashids::decode($request['kegiatan_id']);
     ...
     */
    public function delete_action(Request $request, $kegiatan_id)
    {
        $kegiatan_id = Hashids::decode($kegiatan_id);
        ...
        if (!empty($kegiatan_id)) {
            ...
            $cek_role = DataKegiatan::where('kegiatan_id', $kegiatan_id[0])->first();

            if ($cek_role) {

                DB::beginTransaction();

                $insert['kegiatan_status'] = "N";
                $insert['updated_by'] = Auth::id();

                $delete = $cek_role->update($insert);
