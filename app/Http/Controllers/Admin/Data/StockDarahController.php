...
use App\Model\Data\DataStockDarah;
...
class StockDarahController extends Controller
{
    ...
    * Silahkan ganti 
    * DataStockDarah:: => dengan model anda
    * $stock_id => ganti dengan id di model anda
    */
     ...
     */
    private $title = 'App | Data - Stock Darah'; /**jangan lupa diganti*/
    private $route = 'admin.data.stock_darah.'; //path awal foldernya ajah (misal folder di admin/dashboard) => 'admin.dashboard' | jangan lupa diganti
    ...
    public function getData()
    {
        ...
        $query = DataStockDarah::All();
        foreach ($query as $key => $value) {
            $value->no = $key + 1;
            $value->gol_darah = $value->golongan_darah()->selectRaw('golongan_id, golongan_name')->first();
            $value->prod_darah = $value->produk_darah()->selectRaw('produk_id, produk_name')->first();
        }
        // dd($query);
        return Datatables::of($query)
            ->addColumn('aksi', function (DataStockDarah $dataStockDarah) {
                return "
                        <a href='" . route($this->route . 'update', ['stock_id' => Hashids::encode($dataStockDarah->stock_id)]) . "' class='btn btn-sm btn-primary btn-edit'>Edit</a>
                        <a href='javascript:;' data-route='" . route($this->route . 'delete_action', ['stock_id' => Hashids::encode($dataStockDarah->stock_id)]) . "' class='btn btn-danger btn-sm btn-delete'>Delete</a>
                       ";
            })
            ->addColumn('show', function (DataStockDarah $dataStockDarah) {
                if ($dataStockDarah->stock_show == 'FALSE') {
                    return "<span class='badge badge-danger'>Tidak Aktif</span>";
                } else {
                    return "<span class='badge badge-success'>Aktif</span>";
                }
            })
            ->rawColumns(['aksi','show'])
            ->toJson();
    }
    ...
    */
    ...
    public function create_action(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            DB::beginTransaction();
            $query = DataStockDarah::create($request->input());
    ...
    public function update($stock_id)
    {
        ...
        $stock_id = Hashids::decode($stock_id);
        if (!empty($stock_id)) {

            $cek_data = DataStockDarah::where('stock_id', $stock_id[0])->first();
            $cek_data->gol_darah = $cek_data->golongan_darah()->selectRaw('golongan_id, golongan_name')->first();
            $cek_data->prod_darah = $cek_data->produk_darah()->selectRaw('produk_id, produk_name')->first();
    ...
    {
        $rules = [
            'stock_jumlah'      =>      'required|numeric',
        ];
        $alert = [
            'required'          =>      'The :attribute is required',
            'numeric'           =>      'Stock Jumlah Hanya Berisi Angka'
        ];
        $validator = Validator::make($request->all(), $rules, $alert);

        $stock_id = Hashids::decode($request['stock_id']);
     ...
     */
    public function delete_action($stock_id)
    {
        $stock_id = Hashids::decode($stock_id);
        ...
        if (!empty($stock_id)) {
            ...
            $cek_role = DataStockDarah::where('stock_id', $stock_id[0])->first();
